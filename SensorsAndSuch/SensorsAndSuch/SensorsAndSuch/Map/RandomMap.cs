...
namespace SensorsAndSuch.Maps
{
    ...
    public class RandomMap
    {
        ...
        protected int PosMod = 7;
        public Vector2 TranslateToPos(Vector2 farseerPos)
        {
            Player player= Globals.player;
            Vector2 temp = new Vector2(((int)player.circle.Position.X) / PosMod, ((int)player.circle.Position.Y) / PosMod).Times(PosMod);
            //gridIndex = gridIndex - player.GridPos + new Vector2(20, 15);
            temp = farseerPos - temp;// +new Vector2(20, 15);

            return temp * 100f + new Vector2(BaseTile.TileWidth * 50f, BaseTile.TileHeight*50f);
        }

        public Vector2 TranslateToPosRev(int X, int Y)
        {
            //X = X - (int) (BaseTile.TileWidth * 50f);
            //Y = Y - (int) (BaseTile.TileHeight * 50f);

            Player player = Globals.player;
            Vector2 temp = new Vector2(((int)player.circle.Position.X) / PosMod, ((int)player.circle.Position.Y) / PosMod);
            float temp1 = X /((100 * BaseTile.TileWidth));
            float temp2 = ( temp.X * ((float)PosMod / (BaseTile.TileWidth)));

            float Xt = temp1 + temp2;

            temp1 = Y / ((100 * BaseTile.TileHeight));
            temp2 = (temp.Y * ((float)PosMod / (BaseTile.TileHeight)));

            return new Vector2(Xt, temp1 + temp2);
        }
        ...
        {
            Vector2 CreatePos = TranslateToPosRev(X, Y);
            X = (int) CreatePos.X; 
            Y = (int) CreatePos.Y;
            if (isInBounds(X, Y))
            {
