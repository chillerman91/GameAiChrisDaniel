...
namespace FarseerPhysics.SamplesFramework
{
    internal class SimpleDemo9 : PhysicsGameScreen, IDemoScreen
    {
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            Vector2 rayStart = new Vector2(Position.X, Position.Y); // at the bottom of the player sprite
            Vector2 rayEnd = rayStart + new Vector2(0, -5); // check 5 pixels down
            bool _touchingFloor;
            World.RayCast((fixture, point, normal, fraction) =>
            {
                if (fixture != null)
                {
                    _rectangle[0].Position = point ; //+= new Vector2(0.0f, 0.01f);
                    _rectangle[1].Position = rayStart;
                    _touchingFloor = true;
                    return 1;
                }
                return fraction;
            ...
            ScreenManager.LineBatch.Begin(Camera.SimProjection, Camera.SimView);
                _rectangle[0].Position += new Vector2(0.0f, 0.01f);
            for (int i = 0; i < _ramps.Count; ++i)
            {
                //Shape newst = new Shape
